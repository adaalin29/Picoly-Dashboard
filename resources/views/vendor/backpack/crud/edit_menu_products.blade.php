				...
				<div class="btn-group pull-right">
				  ...
				  <ul class="dropdown-menu">
				  	@foreach ($crud->model->getAvailableLocales() as $key => $locale)
					  	<li><a href="{{ url($crud->route.'/'.$entry->getKey().'/edit') }}?locale={{ $key }}&id_category={{request()->id_category}}&layout={{request()->layout}}">{{ $locale }}</a></li>
				  	@endforeach
				  </ul>
				</div>
		    </div>
...
@push('after_scripts')
  <script>
    $(document).ready(function(){
      $('.file-clear-button').click(function(){
        $("<input type='hidden' name='clear_images[]' value='"+$(this).data('filename')+"'>").insertAfter("input[name=_method]");
      });
